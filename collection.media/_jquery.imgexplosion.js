...
(function($, window, document, undefined) {
    ...
    imgExplosion.prototype.init = function() {
        ...
        $('#qa').append(this.img); // add it to the body to load the image
        // cache measurements
    ...
    imgExplosion.prototype.throwClone = function() {
        ...
        var max = (this.imgW+30 < o.max)? this.imgW+30 : o.max;
        var w = this.randBetween(o.min,max), // new width
    ...
    imgExplosion.defaults = {
        ...
        min: 80,
        // maximum random width of the image in pixels
        max: 300,
...
function reinforcement(){
        ...
        var images = ["_starcircle.png", '_star.png', '_star2.png', '_football.png', '_smiley.png', '_smiley3.png', '_basketball.png'];
        randomNum = 0;
        var image = images[Math.floor(Math.random()*images.length)];
        var booleanVal = [true, false];
        var _explode = booleanVal[Math.floor(Math.random()*booleanVal.length)];
        var _rot = booleanVal[Math.floor(Math.random()*booleanVal.length)];
        var _angle = booleanVal[Math.floor(Math.random()*booleanVal.length)];
        var _alternate = booleanVal[Math.floor(Math.random()*booleanVal.length)];
        var _inPlace = booleanVal[Math.floor(Math.random()*booleanVal.length)];
        var _start = ['top', 'bottom'];
        _start = _start[Math.floor(Math.random()*_start.length)];
        if(randomNum == 0){
            $.fn.imgExplosion({
                img: image,
                start: "top",
